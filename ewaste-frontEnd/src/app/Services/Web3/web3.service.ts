  ...
  public async web3login() {
    ...
    return new Promise( async ( resolve, reject ) => {
      ...
      if ( window.ethereum || window.web3 ) {
        ...
        if ( typeof EWASTEJSON.networks[ Net ] === 'undefined' ) {
        ...
        this.AccountSubscription = this.RefreshedAccount.subscribe( async () => {
          ...
          if (
            ...
            const EWASTEInstance = new window.web3.eth.Contract(
              EWASTEJSON.abi,
              EWASTEJSON.networks[ Network ].address
            )
            this.Web3Details$.next( {
              account: Account,
              network: Network,
              ewaste: EWASTEInstance.methods
            } )
          }
